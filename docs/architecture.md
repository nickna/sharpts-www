...
generate-site.ts + .resx   ── SharpTS interpreter ──> localized HTML/CSS
browser/site.ts            ── TypeScript + esbuild ──> browser JS/CSS
```
...
`execution-contract.ts` contains the application-owned request, response, and
worker payload types plus normalization helpers. Operating-system APIs remain at
the edges because SharpTS supplies its own Node-compatible runtime types. The
browser validates the JSON response again before rendering it.

## Static generation and localization

The generator treats English `.resx` files as the neutral resource set and
requires identical keys for French, Spanish, German, and Simplified Chinese.
It emits direct path-based routes rather than selecting culture at request time.
CSS sources are concatenated deterministically; browser dependencies and fonts
are bundled locally so the production Content Security Policy needs no CDN.
