...
Assert-True (@($manifest.routes).Count -eq 234) "Expected 234 generated routes."
...
Assert-True (Test-Path -LiteralPath (Join-Path $resolvedRoot "performance.json") -PathType Leaf) `
    "Normalized performance data is missing."
...
Assert-True (-not [string]::IsNullOrWhiteSpace($manifest.browserEntry.performanceScript)) `
    "Performance browser entry is missing from the manifest."
...
foreach ($route in $manifest.routes) {
    ...
    if ($route.page -eq "performance") {
        Assert-True ($html.Contains('data-performance-explorer')) `
            "Performance explorer is missing from $($route.route)."
        Assert-True ($html.Contains("src=`"/$($manifest.browserEntry.performanceScript)`"")) `
            "Performance route $($route.route) is missing its explorer bundle."
        Assert-True ($html.Contains('id="compiler"')) "Compiler evidence section is missing."
        Assert-True ($html.Contains('id="desktop"')) "Desktop budget section is missing."
        Assert-True ($html.Contains('id="methodology"')) "Methodology section is missing."
        Assert-True ($html.Contains('href="/performance.json"')) "Raw performance JSON link is missing."
    }
...
foreach ($selector in @('.hero', '.features-grid', '.pipeline', '.playground', '.lang-selector', '.conformance', '.performance-hero', '.performance-ratio', '.docs-layout', '.docs-feedback', '.api-search', '.api-table', '.footer')) {
...
$performanceJavaScriptPath = Join-Path $resolvedRoot $manifest.browserEntry.performanceScript
...
Assert-True (Test-Path -LiteralPath $performanceJavaScriptPath -PathType Leaf) `
    "Performance JavaScript bundle is missing."
...
Assert-True (@($apiSearch.symbols).Count -eq 198) "Expected 198 searchable API symbols."
