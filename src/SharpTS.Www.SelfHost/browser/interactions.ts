...
function findCopyText(button: HTMLButtonElement): string | null {
    ...
    const install = button.closest('.hero__install')?.querySelector('.hero__install-cmd');
    return install?.textContent || null;
}

async function writeClipboard(doc: Document, win: Window, text: string): Promise<void> {
...
function initializeCopyButtons(doc: Document, win: Window): void {
    doc.querySelectorAll<HTMLButtonElement>('[data-copy-button]').forEach(button => {
        let resetTimer: number | undefined;
        button.addEventListener('click', async () => {
            const text = findCopyText(button);
            if (text === null)
                return;
            try {
                await writeClipboard(doc, win, text);
            } catch {
                return;
            }
